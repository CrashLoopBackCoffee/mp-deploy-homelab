...
| Home Assistant metrics | Yes | Configured | Alloy has a static HTTPS scrape for `ha.mpagel.de/api/prometheus` with bearer-token auth loaded from a Kubernetes Secret. |
...
| Home Assistant | Entity and automation metrics exposed by Home Assistant's Prometheus integration | Yes | Configured through an explicit Alloy static HTTPS scrape of `ha.mpagel.de/api/prometheus` with bearer-token auth loaded from a Kubernetes Secret. |
